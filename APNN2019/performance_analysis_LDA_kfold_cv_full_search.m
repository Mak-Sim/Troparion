...
for N=1:N_var
    ...
    [acc_mean(N),acc_std(N), sens_mean(N),sens_std(N), spec_mean(N),spec_std(N)] = ...
        LDA_k_fold_CV(X(:,feature_use),y,K);   
    avg_rec(N) = (sens_mean(N)+spec_mean(N))/2;
    
    result_LDA(N).feature  = feature_use;
    result_LDA(N).acc_mean = acc_mean(N);
    result_LDA(N).acc_std  = acc_std(N);
    result_LDA(N).sens_mean= sens_mean(N);
    result_LDA(N).sens_std = sens_std(N);
    result_LDA(N).spec_mean= spec_mean(N);
    result_LDA(N).spec_std = spec_std(N);
    result_LDA(N).avg_rec  = avg_rec(N);
    
    fprintf('Number of featues %d/%d \n',N,N_var);
end

save('result_LDA_apnn.mat','result_LDA');
