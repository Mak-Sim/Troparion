...
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.163,0.080,0.098,1.695,0.949,1.053,1.210]; i = i + 1;

dataset_praat_h(i).file_name = '004.wav';
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.669,0.387,0.406,5.459,3.000,3.570,3.878]; i = i + 1;

dataset_praat_h(i).file_name = '006.wav';
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.416,0.235,0.219,1.854,0.940,1.036,1.642]; i = i + 1;
...
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.722,0.440,0.407,7.922,4.624,4.749,5.277]; i = i + 1;
...
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.595,0.343,0.337,7.078,4.407,3.956,4.630]; i = i + 1;

dataset_praat_h(i).file_name = '030.wav';
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.875,0.478,0.483,7.832,4.582,4.506,5.562]; i = i + 1;

dataset_praat_h(i).file_name = '033.wav';
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.651,0.336,0.367,3.690,1.757,2.190,3.202]; i = i + 1;
...
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.531,0.288,0.309,3.540,1.870,2.043,3.117]; i = i + 1;
...
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [0.918,0.563,0.441,4.448,2.473,2.480,3.362]; i = i + 1;
...
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [1.557,0.913,0.826,8.472,4.817,5.022,5.813]; i = i + 1;
...
dataset_praat_h(i).label = 0;
dataset_praat_h(i).feature_vec = [1.769,1.033,0.774,5.688,3.115,3.087,4.038]; i = i + 1;
...
% % 
% % % Relabeling and copy
% % dataset_praat = dataset_praat_p;
% % N = length(dataset_praat_p);
% % 
% % for k=1:length(dataset_praat_h)
% %     dataset_praat(N+k) = dataset_praat_h(k);
% %     if(dataset_praat(N+k).label==0)
% %         dataset_praat(N+k).label = -1;
% %     end
% % end
% % 
% % k=0;
% % for i=1:length(dataset_praat)
% % %     v = dataset_irapt(i).feature_vec;
% %     
% %     if (dataset_praat(i).vowel==2) % /a/
% %         if (dataset_praat(i).isBulbar && (dataset_praat(i).label==1))||(dataset_praat(i).label==-1)
% %         k = k+1;
% %         dataset_praat(k) = dataset_praat(i); 
% %         v = dataset_praat(k).feature_vec;
% % %         v =v(4:7);    % all shimmers
% % %         v =v(1:3);    % all jitters
% % %         v = v([1 3 4 7]);
% %         v = v([3 4 7]);
% %         dataset_praat(k).feature_vec = v;
% %         end
% %     end
% % end
% % dataset_praat = dataset_praat(1:k);
